...
import { User, Shield, Bell, Users, AlertCircle, Edit, Trash2 } from 'lucide-react';
...
export function UserSettings() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      setUsers(usersData.filter((u: UserType) => u.role === 'associate' || u.role === 'admin'));
    } catch (error: any) {
  ...
  const handleAddUser = async (userData: any) => {
    try {
      ...
      if (!response.ok) {
        ...
        toast({ title: 'Error', description: errorData.error || `HTTP error! status: ${response.status}`, variant: 'destructive' });
        return false;
      }
  ...
  const recentActiveUsers = users.filter((u) => {
    ...
    try {
      const diffHours = Math.floor((Date.now() - new Date(u.last_login_at).getTime()) / (1000 * 60 * 60));
      return diffHours < 24;
    } catch { return false; }
  }).length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <Card className="hover:border-primary/50 transition-colors">
          <CardContent className="p-6">
            ...
            <p className="text-sm text-muted-foreground mt-1">{activeUsers} active associates</p>
          </CardContent>
        </Card>
        ...
        <Card className="hover:border-accent/50 transition-colors">
          <CardContent className="p-6">
            ...
            <p className="text-sm text-muted-foreground mt-1">{activeUsers} out of {totalUsers} associates active</p>
          </CardContent>
        </Card>
        ...
        <Card className="hover:border-warning/50 transition-colors">
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-bold text-foreground">{recentActiveUsers}</p>
            <p className="text-sm text-muted-foreground mt-1">Associates logged in last 24h</p>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Manage associate access and permissions.
              </p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-muted/50 border-y border-border">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Associate</th>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-1 max-w-xs">
                            {getPermissionBadges(user).map((permission, idx) => (
                              <Badge key={idx} variant="secondary" className="text-xs">{permission}</Badge>
                            ))}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
